...
def run_pipeline(requirements_text, deploy=True, document=True, project_name=None):
    ...
    """
    logger.info("=" * 60)
    logger.info("AGENTIC N8N WORKFLOW PIPELINE - STARTING")
    logger.info("=" * 60)
    ...
    try:
        ...
        workflow_data = generate_workflow(req_path, output_dir)
        results["stages"]["workflow_generation"] = {
            "status": "success",
            "output": f"{output_dir}/workflow.json",
        ...
        if deploy:
            ...
            deployment_info = deploy_to_n8n(workflow_path, "staging", output_dir)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Agentic N8N Workflow Pipeline')
